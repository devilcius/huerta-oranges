...
// .wrangler/tmp/pages-tB7hys/functionsWorker-0.9506273490333457.mjs
...
async function onRequest(context) {
  ...
  if (request.method === "PATCH") {
    ...
    await env.DB.prepare(
      ...
      WHERE id = ?1
      `
    ).bind(buyerId, patchBuyerName, patchBagsOfTen, patchBagsOfTwenty, patchPicked, patchPaid).run();
...
__name(clampNonNegativeInteger, "clampNonNegativeInteger");
__name2(clampNonNegativeInteger, "clampNonNegativeInteger");
async function onRequest2(context) {
  ...
  if (request.method === "POST") {
    ...
    const bagsOfTen = clampNonNegativeInteger2(body.bagsOfTen);
    const bagsOfTwenty = clampNonNegativeInteger2(body.bagsOfTwenty);
    ...
    await env.DB.prepare(
      ...
      `
    ).bind(id, buyerName, bagsOfTen, bagsOfTwenty, createdAtIso).run();
    return json2(
      {
        id,
        buyerName,
...
__name(json2, "json2");
__name2(json2, "json");
function clampNonNegativeInteger2(value) {
  const numberValue = Number(value);
  if (!Number.isFinite(numberValue)) return 0;
  return Math.max(0, Math.trunc(numberValue));
}
__name(clampNonNegativeInteger2, "clampNonNegativeInteger2");
__name2(clampNonNegativeInteger2, "clampNonNegativeInteger");
var routes = [
  {
...
// .wrangler/tmp/bundle-7PqPHC/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-7PqPHC/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.9506273490333457.js.map
